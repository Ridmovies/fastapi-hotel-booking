...
class HotelService(BaseService):
    ...
    async def get_hotels_by_location_objects(
        cls, session: AsyncSession, location: str, date_from: date, date_to: date
    ):
        """Возвращает все отели по заданным параметрам местоположения
        и времени."""
        booked_rooms = (
            select(
                Booking.room_id, func.count(Booking.room_id).label("count_booked_rooms")
            )
            .select_from(Booking)
            .where(
                or_(
        ...
        hotels_with_rooms = (
            select(Hotel.__table__.columns, booked_hotels.c.available_rooms)
    ...
    async def get_all_hotel_rooms_objects(
        cls, session: AsyncSession, hotel_id: int, date_from: date, date_to: date
    ):
        ...
        get_rooms = (
            select(
                ...
                (Room.price_per_day * (date_to - date_from).days).label(
                    "preliminary_cost"
                ),
            )
            .join(booked_rooms, booked_rooms.c.room_id == Room.id, isouter=True)
